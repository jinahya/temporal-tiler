 ...
 * {@link ChronoUnit#DECADES} grain using {@link LocalDate}.
 *
 * @see TemporalTiler
 */
@Slf4j
class TemporalTiler_Decades_Test {

    @Test
    void _PartialHeadAndTail() {
        // ------------------------------------------------------------------------------------------------------- given
        final var start = LocalDate.of(2025, 6, 15);
        final var end = LocalDate.of(2053, 3, 10);
        final var grain = ChronoUnit.DECADES;
        // -------------------------------------------------------------------------------------------------------- when
        ...
        assertTile(tiles.getFirst())
                ...
                .hasEnd(LocalDate.of(2030, 1, 1))
                .isNotAligned();
        assertTile(tiles.get(1))
                .hasStart(LocalDate.of(2030, 1, 1))
                .hasEnd(LocalDate.of(2040, 1, 1))
                .isAligned();
        assertTile(tiles.get(2))
                .hasStart(LocalDate.of(2040, 1, 1))
                .hasEnd(LocalDate.of(2050, 1, 1))
                .isAligned();
        assertTile(tiles.getLast())
                .hasStart(LocalDate.of(2050, 1, 1))
                .hasEnd(LocalDate.of(2053, 3, 10))
                .isNotAligned();
    }
    ...
    void _AllAligned() {
        ...
        final var end = LocalDate.of(2050, 1, 1);
        final var grain = ChronoUnit.DECADES;
        // -------------------------------------------------------------------------------------------------------- when
        final var tiles = TemporalTiler.tile(start, end, grain);
        // -------------------------------------------------------------------------------------------------------- then
    ...
    void _SmallerThanGrain() {
        ...
        final var end = LocalDate.of(2029, 9, 20);
        final var grain = ChronoUnit.DECADES;
        // -------------------------------------------------------------------------------------------------------- when
        final var tiles = TemporalTiler.tile(start, end, grain);
        // -------------------------------------------------------------------------------------------------------- then
